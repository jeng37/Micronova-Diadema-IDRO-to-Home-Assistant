...
CONF_MEMORY_ADDRESS_SENSOR1 = "memory_address_sensor1"
CONF_MEMORY_ADDRESS_SENSOR2 = "memory_address_sensor2"
...
CONFIG_SCHEMA = cv.Schema(
    {
        ...
        cv.Optional(CONF_MEMORY_ADDRESS_SENSOR1): sensor.sensor_schema(
            MicroNovaSensor,
        ).extend(
            MICRONOVA_LISTENER_SCHEMA(
                default_memory_location=0x00, default_memory_address=0x00
        ...
        cv.Optional(CONF_MEMORY_ADDRESS_SENSOR2): sensor.sensor_schema(
            MicroNovaSensor,
        ).extend(
            MICRONOVA_LISTENER_SCHEMA(
                default_memory_location=0x00, default_memory_address=0x00
...
async def to_code(config):
    ...
    if memory_address_sensor_config1 := config.get(CONF_MEMORY_ADDRESS_SENSOR1):
        sens = await sensor.new_sensor(memory_address_sensor_config1, mv)
        cg.add(mv.register_micronova_listener(sens))
        cg.add(
            sens.set_memory_location(memory_address_sensor_config1[CONF_MEMORY_LOCATION])
        )
        cg.add(
            sens.set_memory_address(memory_address_sensor_config1[CONF_MEMORY_ADDRESS])
        )
        cg.add(
            sens.set_function(MicroNovaFunctions.STOVE_FUNCTION_MEMORY_ADDRESS_SENSOR)
        )
    if memory_address_sensor_config2 := config.get(CONF_MEMORY_ADDRESS_SENSOR2):
        sens = await sensor.new_sensor(memory_address_sensor_config2, mv)
        cg.add(mv.register_micronova_listener(sens))
        cg.add(
            sens.set_memory_location(memory_address_sensor_config2[CONF_MEMORY_LOCATION])
        )
        cg.add(
            sens.set_memory_address(memory_address_sensor_config2[CONF_MEMORY_ADDRESS])
        )
        cg.add(
            sens.set_function(MicroNovaFunctions.STOVE_FUNCTION_MEMORY_ADDRESS_SENSOR)
        )
